...
function invaildYears(yearsInput){
  ...
  return yearsInput.trimStart() === '' || !(Number.isInteger(Number(yearsInput))) || parseInt(yearsInput) <= 0;

}

function invaildMonthes(monthesInput){
  
  return !(Number.isInteger(Number(monthesInput))) || parseInt(monthesInput) < 0;

}

function monthlyPaymentCalculation(loanAmount,monthlyInterestRate,loanDuration){
  
   let monthlyPayment;
   return  monthlyPayment = loanAmount * (monthlyInterestRate / (1 - Math.pow((1 + monthlyInterestRate), (-loanDuration))));
...
do{
    ...
    let monthlyInterestRate = (Number(annualPercentageRate)*0.01)/ loanDuration;
    ...
    if (monthlyInterestRate === 0){
      ...
      monthlyPayment = monthlyPaymentCalculation(loanAmountInNumber,monthlyInterestRate,loanDuration);
      //calculating total interest
      let remainingLoan = loanAmountInNumber;
      let interestPerMonth;
      for(let i= 0; i< loanDuration.length; i++){
         remainingLoan 
      }
    }
